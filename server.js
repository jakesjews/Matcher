...
filterResults = (users, uid) ->
  ...
  for user in users
    ...
    user.percent = 100 if user.name is "Amy Grace Standel"
...
calculateRelationship = (user) ->
  ...
  user.percent += 20 if isSingle and user.percent + 20 < 100

calculateMutualFriends = (user) ->
  increaseBy = user.mutual_friend_count * 0.4
  user.percent += increaseBy unless user.percent + increaseBy > 100
